...
typedef struct item
{
	...
	int stu[100] = { 0};				//参加该项目的学生名单，以学号保存
...
//注册一个比赛项目
void register_item(int item_id, char* item_name, int item_nature, char* item_time, char* item_location) {
	ITEMNODE* pNode = (ITEMNODE*)malloc(sizeof(ITEMNODE));	 //申请一个项目
	pNode->item_id = item_id;
...
//项目报名
void sign_up_item(STUNODE* pNode, int item_id) {
	
	for (int i = 0; i < 3; i++) {
		//if (pNode->item_score[i][])
	}
}
